	...
	commands = {
		...
		nick: {
			...
			fn: function (name) {
				...
				if (!name || typeof name !== 'string' || arguments.length !== 1) {
					socket.emit('status', {
						message: 'Error: invalid arguments'
					});
					return;
				...
				if (name !== socket.info.name) {
					socket.broadcast.to(socket.info.room).emit('status', {
						message: socket.info.name + ' has changed their name to ' + name
					});
		...
		list: {
			...
			fn: function () {
				...
				var rooms = objUtil.toArray(io.sockets.adapter.rooms)
					...
					.map(function (r) { return { name: r.key, count: objUtil.toArray(r.value).filter(function (p) { return p.key !== 'length'; }).length };})
					.filter(function (r) { return r.name === 'lobby' || r.count > 0; })
